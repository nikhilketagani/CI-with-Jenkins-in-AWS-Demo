node('jenkinsslave22') {
   ...
   stage('SonarQube analysis') {
     ...
     withSonarQubeEnv('sonarqube-server') {
       sh "${scannerHome}/bin/sonar-scanner -Dsonar.host.url=http://34.93.164.55/ -Dsonar.login=6c64b9c6e7a0c8449cdea0acbe023a47a4d1bd2d -Dsonar.projectVersion=1.0 -Dsonar.projectKey=PROJECT -Dsonar.sourceEncoding=UTF-8 -Dsonar.sources=/var/lib/jenkins/workspace/pipeline/project/src -Dsonar.jacoco.reportPaths=/var/lib/jenkins/workspace/pipeline/project/target/jacoco.exec -Dsonar.junit.reportPaths=/var/lib/jenkins/workspace/pipeline/project/target/surefire-reports/ -Dsonar.java.binaries=**/target/classes"
